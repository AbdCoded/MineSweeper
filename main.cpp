...
public:
	...
	void highscore(string s, int n, int check)
	{
		...
		if (check == 1)
		{
			if (to_string(n) < score[0])
			{
				system("cls");
				cout << "yahoo! you scored a new highscore\n";
				string s;
				cout << "enter your name\n";
				cin >> s;
				for (int i = 4; i > 0; i--)
					score[i] = score[i - 1];
				score[0] = s + ':' + to_string(n);
		...
		int count = 1;
		cout << "high scorers are\n";
		for (int i = 0; i < 5; i++)
		{
			cout << count++ << ". " << score[i] << endl;
		}
			...
			while (choose_difficulty.pollEvent(evnt)) {
				...
				if (Mouse::isButtonPressed(Mouse::Left)) {
					...
					if (beginner_text.getPosition().x <= x + 1 && beginner_text.getPosition().x >= x - 250 && beginner_text.getPosition().y <= y + 1 && beginner_text.getPosition().y >= y - 50) {
						...
						game_over = 0;
						first_move = false;
						setdifficulty("beginner_high.txt");
						game();
					}
					if (intermediate_text.getPosition().x <= x + 1 && intermediate_text.getPosition().x >= x - 330 && intermediate_text.getPosition().y <= y + 1 && intermediate_text.getPosition().y >= y - 50) {
						...
						game_over = 0;
						first_move = false;
						setdifficulty("intermediate_high.txt");
						game();
					}
			...
			while (game.pollEvent(evnt)) {
				...
				case Event::MouseButtonReleased:
					...
					if (count == mine_count) {
						game_over = 2;
						emoji.setTexture(boss);
						int score = clock.getElapsedTime().asSeconds();
						highscore(getdifficulty(), score, 1);
					}
					break;
				}
			}
